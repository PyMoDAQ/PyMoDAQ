...
import PyMoDAQ.DAQ_Utils.python_lib as mylib
...
def h5tree_to_QTree(h5file,base_node,base_tree_elt=None):
    """
    ...
    for node in h5file.list_nodes(base_node):
        ...
        if isinstance(node, tables.Group):
            h5tree_to_QTree(h5file,node,child)

        base_tree_elt.addChild(child)

    return base_tree_elt
...
class DAQ_Analysis(QtWidgets.QWidget,QObject):
    ...
    def update_status(self,txt,wait_time=1000):
        """
...
#to here (see do_load.py)
    ...
    def populate_tree(self):
        ...
        try:
            if self.h5file is not None:
                ...
                base_tree_item=h5tree_to_QTree(self.h5file,base_node)
    ...
    def show_h5_attributes(self,item,col):
        ...
        """
        try:
            node=self.h5file.get_node(item.text(1))
            attrs=node._v_attrs
...
if __name__ == '__main__':
    ...
    win.show()
    prog.open_5h_file('C:\\Data\\2018\\20180212\\Dataset_20180212_000\\Dataset_20180212_000.h5')
    sys.exit(app.exec_())