...
from PyQt5.QtCore import QObject, pyqtSlot, pyqtSignal, QLocale, Qt, QDate, QDateTime, QTime
...
def walk_parameters_to_xml(parent_elt=None,param=None):
    ...
    for param in params_list:
        ...
        elt=ET.Element(param.name(),title=title,type=param_type,visible=visible)
...
def walk_xml_to_parameter(params=[],XML_elt=None):
    ...
    for el in list(XML_elt):
        ...
        if 'group' not in param_type: #covers 'group', custom 'groupmove'...
            ...
            if param_type=='list':
                param=dict(name=el.tag,title=title,type=param_type,value=param_value,values=[param_value],visible=visible)
            else:
                param=dict(name=el.tag,title=title,type=param_type,value=param_value,visible=visible)
        else:
            subparams=[]

            param=dict(name=el.tag,title=title,type=param_type, visible=visible, children=walk_xml_to_parameter(subparams,el))
...
class WidgetParameterItemcustom(pTypes.WidgetParameterItem):
    ...
    def hideEditor(self):
        ...
        """
        if not isinstance(self.widget, QLED):
            self.widget.hide()
...
class file_browser(QtWidgets.QWidget):
    ...
    def __init__(self,init_path='D:/Data'):
        QLocale.setDefault(QLocale(QLocale.English, QLocale.UnitedStates))
