...
from pymodaq.utils.config import get_set_preset_path, GlobalConfig as Config
...
def main():
    ...
    dash = DashBoard(area)
    file = Path(get_set_preset_path()).joinpath(f"{config('utils', 'presets', 'default_preset_for_scan')}.xml")
    if file.exists():
