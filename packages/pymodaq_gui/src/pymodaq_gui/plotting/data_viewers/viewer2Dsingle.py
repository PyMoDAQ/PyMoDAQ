...
from pyqtgraph import ROI as pgROI, HistogramLUTWidget
...
from pymodaq_gui.plotting.items.image import UniformImageItem, SpreadImageItem, PymodaqImage
...
def histogram_factory(image_item=None, gradient='red', levelMode='mono'):
    """
    ...
    histo = pg.HistogramLUTWidget(image_item=image_item, levelMode=levelMode)
...
class ImageDisplayer(QObject):
    ...
    def __init__(self, plotitem,
        ...
        self._image_item: Union[UniformImageItem, SpreadImageItem] = None
    ...
    def set_autolevels(self, isautolevel: bool):
        ...
        self.histogram.item.region.setVisible(not isautolevel)
    ...
    def update_data(self, dwa: DataWithAxes):
        ...
        if self.display_type== 'uniform':
            self._image_item.setImage(np.stack(self._data.data, axis=2),
                                      self.autolevels,
            ...
            data_array = np.stack((nav_axes[0].get_data(),
                                   nav_axes[1].get_data(),
                                   self._data[0]), axis=0).T
            self._image_item.setImage(data_array,
                                      self.autolevels,
                                      levels_sym=self.levels_sym)

    def update_display_items(self, labels: List[str] = None):
        self._plotitem.removeItem(self._image_item)
        if labels is None:
            labels = []
            while len(labels) != len(IMAGE_TYPES):
                labels.append(IMAGE_TYPES[len(labels)])

        self._image_item = image_item_factory(self.display_type)
        self.histogram.item.setImageItem(self._image_item)
        self._plotitem.addItem(self._image_item)
        for ind in range(len(labels)):
            self.legend.addItem(self._image_item, labels[ind])

    def update_image_visibility(self, is_visible: bool):
        self._image_item.setVisible(is_visible)
...
class View2D(ActionManager, QtCore.QObject):
    ...
    def get_theme(name: str = None) -> qt_themes.Theme:
        ...
        if theme is None:
            ...
            theme = themes_dict[list(themes_dict.keys())[0]]
        return theme
    ...
    def setup_lineouts(self):
        self.lineout_viewers: Dict[str, Viewer1D] = (
            dict(hor=Viewer1D(self._lineout_widgets[Lineouts.HOR], show_toolbar=False, no_margins=True),
                 ver=Viewer1D(self._lineout_widgets[Lineouts.VER], show_toolbar=False, no_margins=True,
                              flip_axes=True),
                 int=Viewer0D(self._lineout_widgets[Lineouts.INT], show_toolbar=False, no_margins=True)))
        self.link_lineouts()
    ...
    def set_transform(self, xaxis: Axis, yaxis: Axis):
        ...
        self.data_displayer.get_image().setTransform(tr)
    ...
    def set_image_displayer(self, data_distribution: DataDistribution):
        ...
        self.connect_action('show_image', self.notify_visibility_data_displayer)
    ...
    def setup_graphs(self, graphs_layout):
        ...
        graphs_layout.addWidget(self.histogram_widget)
        ...
        self.splitter_VLeft.addWidget(self._lineout_widgets[Lineouts.HOR])
        self.splitter_VRight.addWidget(self._lineout_widgets[Lineouts.VER])
        self.splitter_VRight.addWidget(self._lineout_widgets[Lineouts.INT])
    ...
    def setup_actions(self):
        ...
        self.add_action('show_image', 'Show Image', 'bid_landscape', tip='Show/Hide Image Item', checkable=True)
    ...
    def connect_things(self):
        self.connect_action('histo', lambda x: self.histogram_widget.item.regionChanged())
        ...
        self.roi_manager.new_ROI_signal.connect(self.lineout_viewers[Lineouts.INT].get_action('clear').click)
    ...
    def show_hide_histogram(self, show=True):
        self.histogram_widget.setVisible(self.is_action_checked('show_image'))
    ...
    def display_roi_lineouts(self, roi_dte: DataToExport):
        if len(roi_dte) > 0:
            for lineout_type in Lineouts.values():
    ...
    def notify_visibility_data_displayer(self):
        self.data_displayer.update_image_visibility(self.is_action_checked('show_image'))
    ...
    def show_hide_crosshair(self, show=True):
        ...
        self.crosshair.setVisible(show)
        if show:
            self.lineout_viewers[Lineouts.HOR].view.add_data_displayer('crosshair', plot_colors=crosshair_pens)
            self.lineout_viewers[Lineouts.VER].view.add_data_displayer('crosshair', plot_colors=crosshair_pens)
            self.lineout_viewers[Lineouts.INT].view.add_data_displayer('crosshair', plot_colors=crosshair_pens)
            range = self.get_view_range()
            self.set_crosshair_position(np.mean(np.array(range[0])), np.mean(np.array(range[0])))
        else:
            self.lineout_viewers[Lineouts.HOR].view.remove_data_displayer('crosshair')
            self.lineout_viewers[Lineouts.VER].view.remove_data_displayer('crosshair')
            self.lineout_viewers[Lineouts.INT].view.remove_data_displayer('crosshair')
    ...
    def set_image_labels(self, labels: List[str]):
        if self.data_displayer.labels != labels:
            self.get_action('show_image').setToolTip('Show/Hide'
                                                        f' - '
                                                        f'{labels}')
...
class Viewer2D(ViewerBase):
    ...
    def __init__(self, parent: QtWidgets.QWidget = None, title=''):
        ...
        self.filter_from_rois = Filter2DFromRois(self.view.roi_manager,
                                                 self.view.data_displayer.get_image(),
                                                 IMAGE_TYPES)
        self.filter_from_rois.register_activation_signal(
            self.view.get_action('roi').triggered)
        self.filter_from_rois.register_target_slot(self.process_roi_lineouts)

        self.filter_from_crosshair = Filter2DFromCrosshair(
            self.view.crosshair,
            self.view.data_displayer.get_image(),
            IMAGE_TYPES)
        self.filter_from_crosshair.register_activation_signal(
            self.view.get_action('crosshair').triggered)
    ...
    def roi_changed(self, *args, **kwargs):
        self.filter_from_rois.filter_data(self._data)

    def crosshair_changed(self):
        self.filter_from_crosshair.filter_data(self._data)
    ...
    def _show_data(self, data: DataWithAxes, *args, **kwargs):
        ...
        """
        if len(data) == 1:
            self.view.histogram_widget.item.setLevelMode('mono')
        else:
            self.view.histogram_widget.item.setLevelMode('rgba')
        ...
        if data.distribution != self.view.data_displayer.display_type:
            ...
            self.filter_from_crosshair.set_graph_items(self.view.data_displayer.get_image())
    ...
    def update_data(self):
        if self._raw_data is not None:
            self._data = self.set_image_transform()

            self.view.display_images(self._data)
            if self._data.distribution.name == 'uniform':
                xaxis = self._data.get_axis_from_index(1)[0]
                yaxis = self._data.get_axis_from_index(0)[0]
                self.x_axis = xaxis
                self.y_axis = yaxis
                self.view.set_transform(xaxis, yaxis)
            else:
                self.x_axis = self._data.get_axis_from_index(0)[0]
                self.y_axis = self._data.get_axis_from_index(0)[1]
    ...
    def set_visible_items(self):
        self.view.set_action_visible('show_image', self.isdata['red'])
    ...
    def prepare_connect_ui(self):
        ...
        self.view.histogram_widget.item.sigLookupTableChanged.connect(lambda: setattr(self, '_is_gradient_manually_set', True))
    ...
    def process_roi_lineouts(self, roi_dte: DataToExport):
            if len(roi_dte) > 0:
                ...
                for dwa in roi_dte_bis.data:
                    if dwa.name == Lineouts.HOR:
                        dwa.name = f'Hlineout'
                    elif dwa.name == Lineouts.VER:
                        dwa.name = f'Vlineout'
                    elif dwa.name == Lineouts.INT:
...
def generate_uniform_data() -> DataRaw:
    ...
    x0 = 5
    data_red = 213 * np.cos((x-x0) / 5)**2 * gauss2D(x, x0, Nx / 10, y, -1, Ny / 10, 1, 90) + 0.5 * data_random
    data_green = 140 * gauss2D(x, -20, Nx / 10, y, -10, Ny / 20, 1, 0)
    data_green[70:80, 7:12] = np.nan

    data_to_plot = DataRaw(name='mydata', distribution='uniform',
                                   data=[data_red, data_green, data_red-0.3*data_green],
