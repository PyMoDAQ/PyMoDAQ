...
from pymodaq_gui.utils import (
    QLED,
    DockArea,
    PushButtonIcon,
    QSpinBox_ro,
    QSpinBoxWithShortcut,
)
...
class DAQ_Move_UI_Base(ControlModuleUI):
    ...
    def actuators(self):
        return [
            self.actuators_combo.itemText(ind)
            for ind in range(self.actuators_combo.count())
        ]
    ...
    def setup_docks(self):
        ...
        self.abs_value_sb = QSpinBoxWithShortcut(
        ...
        self.abs_value_sb_2 = QSpinBoxWithShortcut(
            step=0.1, dec=True, siPrefix=config("actuator", "siprefix")
        )
        self.abs_value_sb_2.setStyleSheet("background-color : lightcoral; color: black")

        self.abs_value_sb_bis = QSpinBoxWithShortcut(
            step=0.1, dec=True, siPrefix=config("actuator", "siprefix")
        )
        self.ini_actuator_pb = PushButtonIcon(
            "ini",
            "Initialization",
            checkable=True,
        ...
        self.current_value_sb = QSpinBox_ro(
            font_size=20,
            min_height=27,
        ...
        self.move_abs_pb = PushButtonIcon(
            "Move",
            "Set Abs.",
            tip="Set the value of the actuator to the set absolute value",
        )
        self.rel_value_sb = QSpinBoxWithShortcut(
            step=0.1,
            dec=True,
            siPrefix=config("actuator", "siprefix"),
    ...
    def send_init(self, checked):
        ...
        self.command_sig.emit(
            ThreadCommand(
                UiToMainMove.INIT,
                [self.ini_actuator_pb.isChecked(), self.actuators_combo.currentText()],
            )
        )
    ...
    def emit_move_abs(self, spinbox):
        ...
        self.command_sig.emit(
            ThreadCommand(
                UiToMainMove.MOVE_ABS,
                DataActuator(data=spinbox.value(), units=self._unit),
            )
        )
    ...
    def emit_move_rel(self, sign):
        self.command_sig.emit(
            ThreadCommand(
                ...
                DataActuator(
                    data=self.rel_value_sb.value() * (1 if sign == "+" else -1),
                    units=self._unit,
                ),
            )
        )
    ...
    def connect_things(self):
        ...
        if "move_abs" in self.actions_names:
            self.connect_action(
                "move_abs", lambda: self.emit_move_abs(self.abs_value_sb)
            )
        if "move_abs_2" in self.actions_names:
            self.connect_action(
                "move_abs_2", lambda: self.emit_move_abs(self.abs_value_sb_2)
            )
        ...
        self.move_abs_pb.clicked.connect(
            lambda: self.emit_move_abs(self.abs_value_sb_bis)
        )
        self.abs_value_sb.shortcut["Ctrl+E"].activated.connect(
            lambda: self.emit_move_abs(self.abs_value_sb)
        )
        self.abs_value_sb_2.shortcut["Ctrl+E"].activated.connect(
            lambda: self.emit_move_abs(self.abs_value_sb_2)
        )
        self.abs_value_sb_bis.shortcut["Ctrl+E"].activated.connect(
            lambda: self.emit_move_abs(self.abs_value_sb_bis)
        )
        self.rel_value_sb.valueChanged.connect(
            lambda: self.command_sig.emit(
        ...
        self.rel_value_sb.shortcut["Ctrl+E"].activated.connect(
            lambda: self.emit_move_rel("+")
        )
        self.rel_value_sb.shortcut["Ctrl+Shift+E"].activated.connect(
            lambda: self.emit_move_rel("-")
        )

        self.find_home_pb.clicked.connect(
            lambda: self.command_sig.emit(
                ThreadCommand(
                    UiToMainMove.FIND_HOME,
                )
            )
        )
        self.stop_pb.clicked.connect(
            lambda: self.command_sig.emit(
                ThreadCommand(
                    UiToMainMove.STOP,
                )
        ...
        self.get_value_pb.clicked.connect(
            lambda: self.command_sig.emit(
                ThreadCommand(
                    UiToMainMove.GET_VALUE,
                )
            )
        )
        ...
        self.actuators_combo.currentTextChanged.connect(
            lambda act: self.command_sig.emit(
                ThreadCommand(UiToMainMove.ACTUATOR_CHANGED, act)
            )
        )
        ...
        if "refresh_value" in self.actions_names:
            self.connect_action(
                "refresh_value",
                lambda do_refresh: self.command_sig.emit(
                    ThreadCommand(UiToMainMove.LOOP_GET_VALUE, do_refresh)
                ),
            )

    def show_tree(self, show: bool = True):
        self._tree.setVisible(show)
        self._tree.closeEvent = lambda event: self.set_action_checked(
            "show_settings", False
        )

    def show_controls(self, show: bool = True):
        self.control_widget.setVisible(show)
        self.control_widget.closeEvent = lambda event: self.set_action_checked(
            "show_controls", False
        )

    def show_graph(self, show: bool = True):
        self.graph_widget.setVisible(show)
        self.graph_widget.closeEvent = lambda event: self.set_action_checked(
            "show_graph", False
        )
