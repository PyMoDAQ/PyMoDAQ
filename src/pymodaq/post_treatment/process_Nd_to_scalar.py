...
class DataNDProcessorBase(metaclass=ABCMeta):
    ...
    def process(self, limits: List, data: DataWithAxes):
        #todo modify this to apply to ND data
...
class MeanProcessor(Data1DProcessorBase):
    def operate(self, sub_data, sub_axis=None, axis=-1):
        return np.mean(sub_data, axis=axis)


class StdProcessor(Data1DProcessorBase):
    def operate(self, sub_data, sub_axis, axis=-1):
        return np.std(sub_data, axis=axis)


class SumProcessor(Data1DProcessorBase):
    def operate(self, sub_data, sub_axis=None, axis=-1):
        return np.sum(sub_data, axis=axis)


class MaxProcessor(Data1DProcessorBase):
    def operate(self, sub_data, sub_axis=None, axis=-1):
        return np.max(sub_data, axis=axis)


class MinProcessor(Data1DProcessorBase):
    def operate(self, sub_data, sub_axis=None, axis=-1):
        return np.min(sub_data, axis=axis)
...
if __name__ == '__main__':
    ...
    print(f"Math_mean: {processors.get('mean', **config_processors).process((0, 200), ydata_expodec, x)[2]}\n"
          f"Mean tot: {np.mean(ydata_expodec)}")
    sum_processor = processors.get('sum', **config_processors)
    sum_processor.process((50, 200), ydata_expodec, x)

    print(f"FWHM should be 25, calculated at :{processors.get('fwhm').process((0, 200), y1, x)[2]}")
