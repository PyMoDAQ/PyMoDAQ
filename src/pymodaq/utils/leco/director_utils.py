...
from pymodaq.utils.leco.utils import PymodaqMessage, get_pymodaq_data, DeSerializer
...
class GenericDirector(Director):
    ...
    def ask_rpc_pymodaq(self, method: str, pymodaq_data, actor: None = None,
        ...
        message = PymodaqMessage(receiver=self._actor_check(actor), data=string,
                                 pymodaq_data=pymodaq_data)
        response = self.communicator.ask_message(message)
        return_value = self.communicator.rpc_generator.get_result_from_response(response.payload[0])
        if return_value is None:
            return_value = get_pymodaq_data(response)
        return return_value
...
class ActuatorDirector(GenericDirector):
    def move_abs(self, position: Union[float, DataActuator]) -> None:
        if isinstance(position, DataActuator):
            self.ask_rpc_pymodaq(method="move_abs", pymodaq_data=position)
        self.ask_rpc("move_abs", position=position)

    def move_rel(self, position: Union[float, DataActuator]) -> None:
        if isinstance(position, DataActuator):
            self.ask_rpc_pymodaq(method="move_rel", pymodaq_data=position)
