...
class DirectorCommands(StrEnum):
    ...
    SET_UNITS = (
        MoveDirectorMethods.SET_UNITS
    )  # to set units accordingly to the one of the actor
...
leco_parameters = [
    ...
    },
    {
        ...
        "name": "host",
        "type": "str",
        "value": config("network", "leco-server", "host"),
    },
    {
        "title": "Coordinator Port:",
        "name": "port",
        "type": "int",
        "value": config("network", "leco-server", "port"),
    },
    {
        "title": "Settings PyMoDAQ Client:",
...
class LECODirector:
    ...
    def __init__(
        self, host: str = "localhost", port: int = COORDINATOR_PORT, **kwargs
    ) -> None:
        ...
        self.register_rpc_methods((self.set_director_settings,))
        self.register_binary_rpc_methods((self.set_director_info,))
    ...
    def check_actor_connection(self) -> None:
        try:
            ...
            if exc.rpc_error.code == RECEIVER_UNKNOWN.code:
                self.emit_status(
                    ThreadCommand(ThreadStatus.UPDATE_UI, "do_init", args=[False])
                )
            else:
                self.emit_status(
                    ThreadCommand(
