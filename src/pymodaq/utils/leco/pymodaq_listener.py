...
class ActorHandler(PymodaqPipeHandler):
    def register_rpc_methods(self) -> None:
        ...
        self.register_rpc_method(
            self.get_actuator_value, name=MoveMethods.GET_ACTUATOR_VALUE
        )
    ...
    def extract_pymodaq_object(
        ...
        if value is None and additional_payload:
            return cast(
                DataWithAxes,
                SerializableFactory().get_apply_deserializer(additional_payload[0]),
            )
        else:
            return value
    ...
    def set_info(
        ...
        self.signals.cmd_signal.emit(
            ThreadCommand(LECOCommands.SET_INFO, attribute=param)
        )
    ...
    def move_abs(self, position: Union[float, str]) -> None:
        pos = (
            self.extract_dwa_object(position) if isinstance(position, str) else position
        )
...
class PymodaqListener(Listener):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(name, host, port, logger=logger, timeout=timeout, **kwargs)
    ...
    def _listen(
        ...
        self.message_handler = self._handler_class(
            name,
    ...
    def stop_listen(self) -> None:
        ...
        try:
            ...
            pass
        self.signals.cmd_signal.emit(
            ThreadCommand(LECOClientCommands.LECO_DISCONNECTED)
        )

    def indicate_sign_in_out(self, full_name: str):
        if "." in full_name:
            self.signals.cmd_signal.emit(
                ThreadCommand(LECOClientCommands.LECO_CONNECTED)
            )
        else:
            self.signals.cmd_signal.emit(
                ThreadCommand(LECOClientCommands.LECO_DISCONNECTED)
            ...
            if command.command == LECOViewerCommands.DATA_READY:
                ...
                self.send_rpc_message_to_remote(
                    ...
                    **binary_serialization_to_kwargs(
                        command.attribute, data_key="parameter"
                    ),
                )
                ...
                self.send_rpc_message_to_remote(
                    ...
                    **binary_serialization_to_kwargs(
                        pymodaq_object=value, data_key="position"
                    ),
                )
