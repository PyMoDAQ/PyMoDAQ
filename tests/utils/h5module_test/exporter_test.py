...
from pymodaq.utils.h5modules import h5exporters as h5export
...
def create_test_file(request, qtbot):
    ...
    settings_xml = ioxml.parameter_to_xml_string(settings)
...
class TestH5BrowserUtil:
    ...
    def test_get_h5_attributes(self, load_test_file):
        ...
        assert attr_dict['dtype'] == 'float64'
    ...
    def test_get_h5_data(self, load_test_file):
        ...
        node_path = '/Raw_datas/Logger'
        node = h5utils.get_node(node_path)
        assert isinstance(node, backends.StringARRAY)
    ...
    def test_export_h5_data(self, load_test_file):
        ...
        node = h5utils.get_node(node_path)
        data = node.read()
        savepath = str(Path(__file__).parent.joinpath('data/stringdata.txt'))
        h5utils.export_data(node_path, filesavename=savepath)
        ...
        node = h5utils.get_node(node_path)
        data = node.read()
        savepath = str(Path(__file__).parent.joinpath('data/data.txt').resolve())
        h5utils.export_data(node_path, savepath)
        data_back = np.loadtxt(savepath)
        assert np.all(data == pytest.approx(data_back))
        os.remove(savepath)

        node_path = '/Agroup'
        h5utils.export_data(node_path,savepath)  # export both Data and X_axis node
