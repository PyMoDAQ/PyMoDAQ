...
class TestItemSelect:
    ...
    def test_isSelected_setValue(self,init_ParameterTree):
        ...
        for doCheckbox in [True,False]:
            params_itemSelect = {'title': 'Dragable items', 'name': 'itemsSelect_drag', 'type': 'itemselect',
            ...
            settings.setValue(dict(all_items=['item1', 'item2', 'item3'], selected=['item1','item2']))
            assert settings.value() == dict(all_items=['item1', 'item2', 'item3'], selected=['item1','item2'])

            # Removing selection
            settings.setValue(dict(all_items=['item1', 'item2', 'item3'], selected=['item2',]))
            assert settings.value() == dict(all_items=['item1', 'item2', 'item3'], selected=['item2',])

            # Adding selection (non matching order between all/selected)
            settings.setValue(dict(all_items=['item1', 'item2', 'item3'], selected=['item2','item1']))
            assert settings.value() == dict(all_items=['item1', 'item2', 'item3'], selected=['item2','item1'])

            # Adding selection (non matching order between all/selected)
            settings.setValue(dict(all_items=['item1', 'item2', 'item3','item4'], selected=['item2','item1','item3','item4']))
            assert settings.value() == dict(all_items=['item1', 'item2', 'item3','item4'], selected=['item2','item1','item3','item4'])
    ...
    def test_isSelected_clicked(self,init_ParameterTree):
        for doCheckbox in [True,False]:
            ...
            assert settings.value() == dict(all_items=['item1', 'item2', 'item3',], selected=['item3','item1'])

            # Unselecting item
            listwidget.select_item(listwidget.item(2),False)
            assert settings.value() == dict(all_items=['item1', 'item2', 'item3',], selected=['item1'])

            # Reselecting item
            listwidget.select_item(listwidget.item(2),True)
            assert settings.value() == dict(all_items=['item1', 'item2', 'item3',], selected=['item1','item3'])